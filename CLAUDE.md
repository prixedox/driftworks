...
- **Phase 1 (Foundation UX) is complete** — plans #4–#7 all built & live. Next: master-plan **Phase 2 (Tooltips & UI v2)** — see `docs/superpowers/plans/phases/phase-2-tooltips-ui-v2.md` (run its Pre-flight reality check first). Note: Phase 2 removes Explain mode + the dead `EXPLAIN` map, and reorganizes the now-crowded dock (10 buttons) into a More sheet.
- **Shipped:** save v3 (persists inventory/research/unlocks/upgrades) · copper/circuits + `RECIPES` (recipe-select machines) · CI test gating + determinism harness · **Phase 1 Foundation UX**: build UX (erase refund, undo, blueprint copy/paste — 1.1) · settings + graphics-quality tiers (bloom/shadows/particles, localStorage, auto-detect — 1.2) · production stats panel (per-item rates + sparklines, all items) + minimap (MinimapLayer snapshot field — 1.3) · first-run tutorial + objective chip (1.4).
